...
// Today's orders
$stmt = $pdo->query("SELECT COUNT(*) as total FROM pedidos WHERE DATE(created_at) = CURRENT_DATE");
...
// Today's revenue
$stmt = $pdo->query("SELECT COALESCE(SUM(total_geral), 0) as total FROM pedidos WHERE status = 'entregue' AND DATE(created_at) = CURRENT_DATE");
$stats['receita_hoje'] = $stmt->fetch()['total'];

// Active products
$stmt = $pdo->query("SELECT COUNT(*) as total FROM lanches WHERE ativo = TRUE");
$stats['lanches_ativos'] = $stmt->fetch()['total'];

$stmt = $pdo->query("SELECT COUNT(*) as total FROM acompanhamentos WHERE ativo = TRUE");
$stats['acompanhamentos_ativos'] = $stmt->fetch()['total'];

// Active customers
$stmt = $pdo->query("SELECT COUNT(*) as total FROM clientes WHERE ativo = TRUE");
