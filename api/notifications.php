...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($type === 'admin' && isAdmin()) {
        ...
        // Get unread count
        $stmt = $pdo->query("SELECT COUNT(*) as count FROM notificacoes WHERE destinatario = 'admin' AND lida = FALSE");
        ...
        $stmt = $pdo->prepare("
            SELECT * FROM notificacoes 
            WHERE destinatario = 'cliente' AND cliente_id = ? AND lida = FALSE
            ORDER BY created_at DESC
        ");
